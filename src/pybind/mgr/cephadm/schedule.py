...
def get_placement_hosts(
    ...
    if spec.placement.hosts:
        # Precompute the set of draining hostnames to reduce O(N*M) list comp in the loop to O(N+M)
...
class HostAssignment(object):
    ...
    def place_per_host_daemons(
        ...
        # Precompute unreachable hostnames to reduce O(N*M) check to O(N+M)
